...
class ModelConfig(BaseModel):
    ...
    model_class: str = Field("Image", description="Model class identifier")
    ...
    @property
    def hosting_location(self) -> ModelHostingLocation:
        if self.model_type == ModelType.API:
            return ModelHostingLocation.External
        return ModelHostingLocation.Internal

    @model_validator(mode="after")
    def enforce_request_type(self) -> "ModelConfig":
        if self.hosting_location == ModelHostingLocation.Internal:
            self.request_type = HttpRequestType.Post
        return self
